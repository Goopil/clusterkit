...
feat: add `health.maxEventLoopLagMs` — recycle a worker whose reported event-loop lag exceeded this value (ms) for 3 consecutive beats, through the same bounded drain as the other health policies. Catches "slow but alive" workers that the wedged (heartbeat silence) policy never sees. Opt-in (`0` = disabled), requires `health.heartbeatMs > 0`. `worker:recycle` gains the `"lag"` reason.
